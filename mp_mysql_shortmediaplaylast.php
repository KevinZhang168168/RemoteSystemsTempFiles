...
if($link){
    ...
    //参考另外一个php文件，这样问题就解决了
    if (mysqli_fetch_assoc($resultmax)> $idMedia)
    {
        //$idMedia = $idMedia + 1;//如果传进来的id值小于数据库里的最大值，则加一；
        $sql = "SELECT idStudent_shortVideo_log,Student_shortVideo_log_Stuid_FK,Student_shortVideo_logName,Student_shortVideo_logPICURI FROM Student_shortVideo_log where idStudent_shortVideo_log = (select min(idStudent_shortVideo_log) from Student_shortVideo_log where idStudent_shortVideo_log > ".$idMedia.")";
    }
    else{
