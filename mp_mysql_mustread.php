...
//因为字的页面是从古诗页面过去的，所以我把这个古诗里面需要会认的字放到一起了；
//PHP+MySQL测试结果：[ { "Poetrymustread": "壶,酒" } ]，这个没有带拼音，因为突然想起来，这个值其实古诗页面中已经有了，不必再次向服务器请求。
...
if($link){
    ...
    $sql = "SELECT Poetrymustread FROM Poetry where idPoetry=1";
    ...
    if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
            ...
            $poetrycontent->Poetrymustread=$row["Poetrymustread"];
